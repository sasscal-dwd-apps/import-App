...
#'
#' @title Compact the import-App (small version)
#'
#' @description This function creates a new folder called "import_compact_small"
#' that contains all the files required for the installation and run of the
#' import-App. Its size is around 11 Mb
#' 
...
#' package, with which the user will be able to install the import-App. The user
...
compact_small <- function(appDir){
  ...
  appDir_new <<- normalizePath(file.path(appDir, "..", "import_compact_small"))
  ...
  filesToDelete <- list.files(path = appDir_new, pattern = "documentation.html",
                              recursive = T)
